...
- Observed mean velocity tends to increase as Distinctness increases.

- In our dataset, we have more observations at higher distinctness
  levels.

Why might your table differ from Michelson’s?

Our table differs only slightly from Michelson’s because our values have
more significant figures than Michelson’s. For example, For Distinctness
= 1 our table says that the mean velocity = 299808.0 while Michelson’s
mean velocity is 299810.
...
Low_True_error <-  LIGHTSPEED_MICHELSON - LIGHTSPEED_PM
High_True_error <-  LIGHTSPEED_MICHELSON + LIGHTSPEED_PM
...
Michelson’s estimate is less than the true error.
...
- Both graphs have data centered around the range 299900 to 300000
  km/second. Both graphs have variation in data above and below those
  values.

Differences:

- The real data measurement looks like they are generally seeing more
  outliers and getting mean values outside of the 299900 to 300000 range
  in the beginning (pre June 16) and getting more focused measurements
  within the range as time goes on. The simulated data measurement is
  the opposite: The initial measurements are more focused within the
  299900 to 300000 range (starting after June 23) while later on the
  mean values deviate outside the range.
...
ggplot(df_q2)+
  ...
  ggtitle("Michelson's Temp vs. Velocity Vacuum")
```
...
``` r
ggplot(df_q2)+
  geom_boxplot(mapping = aes(x = Distinctness , y = VelocityVacuum, color = Distinctness))+
  ggtitle("Michelson's Distinctness vs. Velocity Vacuum")
```
