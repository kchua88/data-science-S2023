...
pi_est <- df_generated %>% 
  mutate(stats = (x^2+y^2 <= 1)) %>% 
  summarize(pi_approximation = mean(stats)*4)
pi_est
```
...
df_clt <- df_q1 %>% 
  mutate(
    ...
    low = pi_approximation - q95 * standard_error,
    high = pi_approximation + q95 * standard_error
  ) 
df_clt
```
...
- Does your interval include the true value of $\pi$?
  - The interval includes the true value of pi with the lower bound
    being 3.02 and the higher bound being 3.22.
- What confidence level did you choose?
  - I chose a 95% confidence level.
- Was your sample size $n$ large enough? Why do you say that?
  - My sample size of 1000 I think is large enough because it was able
    to produce a standard error of 0.0524 which is under 2% of the value
    of pi. I think this is makes the estimate reasonably close when
    approximating the true value of pi.
